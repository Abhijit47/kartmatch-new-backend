...
export const insertVendors = async (req, res) => {
  try {
    ...
    const validVendors = vendors
      .map((vendor) => {
        const [longitude, latitude] = toCoordinates(
          vendor.latitude,
          vendor.longitude
        );
        ...
        const hygieneRating = isNaN(vendor.hygiene)
          ? vendor.hygiene
          : parseFloat(vendor.hygiene);
        const tasteRating = isNaN(vendor.taste)
          ? vendor.taste
          : parseFloat(vendor.taste);
...
export const fetchVendorsData = async (req, res) => {
  try {
    ...
    const [bengalVendors, maharashtraVendors, rajasthanVendors] =
      await Promise.all([
...
export const getNearbyVendors = async (req, res) => {
  try {
    ...
    const cacheKey = `nearby-${latitude.toFixed(4)}-${longitude.toFixed(
      4
    )}-${maxDistance}`;
...
export const getFilteredVendors = async (req, res) => {
  try {
    ...
    if (!preferences || preferences.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: 'No preferences provided.' });
    }
    ...
    preferences.forEach((pref) => {
      ...
      if (field) {
        ...
        filter[field] = {
          $regex: new RegExp(`^(${minStars}|[${minStars}-5]) star$`, 'i'),
        };
      }
    });
